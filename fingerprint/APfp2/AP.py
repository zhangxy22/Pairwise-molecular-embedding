...
dataxa_train = pd.read_csv('AP_train_a.csv')
dataxd_train = pd.read_csv('AP_train_d.csv')
datay = pd.read_csv('y_train.csv')
datay_train = datay[['PCE']]

dataxa_test = pd.read_csv('AP_test_a.csv')
dataxd_test = pd.read_csv('AP_test_d.csv')
...
ckpt_path = os.path.join(SAVER_DIR, "model2_fp")
...
with tf.Session() as sess:
    ...
    for i in range(100):
        ...
        for start, end in training_batch:
            sess.run(train_op1, feed_dict={X: train_x[start:end], Y: train_y[start:end], p_keep_conv: 0.7})

        loss = sess.run(prediction_error, feed_dict={X: train_x, Y: train_y, p_keep_conv: 0.7})
...
with tf.Session() as sess:
    ...
    print("R = ", r(train_y, y_train_pred))
