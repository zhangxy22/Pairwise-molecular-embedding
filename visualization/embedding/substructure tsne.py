...
def custom_distance(X1, X2, gamma_d, gamma_a):
    ...
    T_d = (np.dot(np.transpose(X1[:ndesp1]), X2[:ndesp1])) / (
                np.dot(np.transpose(X1[:ndesp1]), X1[:ndesp1]) + np.dot(np.transpose(X2[:ndesp1]),
                                                                        X2[:ndesp1]) - np.dot(np.transpose(X1[:ndesp1]),
                                                                                              X2[:ndesp1]))
...
ckpt_path = os.path.join(SAVER_DIR, "model8_fp")
...
with tf.Session() as sess:
    ...
    for j, label in enumerate(labels):
        x, y = low_dim_embs[j, :]
        plt.scatter(x, y)
        plt.annotate(label, xy=(x, y), xytext=(2, 2), textcoords="offset points")  # 给嵌入向量点标注注释

    # 以png格式保存图片
    plt.savefig("after_tsne601.png")
