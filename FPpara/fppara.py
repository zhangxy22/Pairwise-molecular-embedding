...
bit_size = 1024
embedding_size = 100
emb = tf.Variable(tf.random.uniform([bit_size + 1, embedding_size], -1, 1), dtype=tf.float32)
...
def plot_scatter(x_train, y_train, x_test, y_test):
    ...
    ax = plt.subplot(gs[0])
    #ax.scatter(x_train, y_train, color="lightsteelblue", s=10)
    # color1 = (151 / 255, 149 / 255, 235 / 255)
    # ax.scatter(x_train, y_train, s=20, color=color1, alpha=0.5)
    # color2 = (251 / 255, 178 / 255, 41 / 255)
    # ax.scatter(x_test, y_test, s=20, color=color2, alpha=0.3)
    ax.scatter(x_train, y_train, s=20, color='Blue', alpha=0.3)
    ax.scatter(x_test, y_test, s=20, color='orange', alpha=0.7)
...
ckpt_path = os.path.join(SAVER_DIR, "model6")
...
with tf.Session() as sess:
    ...
    for i in range(100):
        ...
        for start, end in training_batch:
            sess.run(train_op1, feed_dict={X: train_x[start:end], Y: train_y[start:end], p_keep_conv: 0.3})
