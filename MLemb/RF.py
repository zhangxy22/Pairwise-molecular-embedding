...
##########################################################################################
def RMSE(a,b):
    RMSE=math.sqrt(np.sum((a-b)**2)/len(a))
    return RMSE
from sklearn.metrics import make_scorer
rmse_loss= make_scorer(RMSE, greater_is_better=False)
r_score = make_scorer(r, greater_is_better=True)
kfold=KFold(n_splits=10,shuffle=True,random_state=2413)
rmse_errors_val=cross_validate(model,x_train,y_train,cv=kfold,scoring=rmse_loss)
r_scores_val=cross_val_score(model,x_train,y_train,cv=kfold,scoring=r_score)
